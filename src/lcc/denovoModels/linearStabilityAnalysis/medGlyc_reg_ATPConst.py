...
KCAT_PK = 170
##
KCAT_SUMMARY_MAKE_PEPASE = 1.2e3 # Arbitrary
# ks
...
#####
KM_SUMMARY_MAKE_PEPASE = uMtoCount(0.03) # just guessing.
...
CONC_SUMMARY_MAKE_PEPASE = uMtoCount(5e-2)
...
# Attempting to base
def hexokinase(Conc_Substrate1 = CONC_GLUCOSE_T0, Conc_Substrate2 = CONC_ATP_T0, Conc_Enzyme = CONC_HEXOKINASE, kcat = KCAT_HEXOKINASE, KM1 = KM_HEXOKINASE_GLUCOSE,KM2 =  KM_HK_ATP):
    # Note: glucose is not decreasing...
    return (kcat * Conc_Enzyme * saturation(Conc_Substrate1, KM1) * saturation(Conc_Substrate2, KM2))

if useOneStatePFK:
    #def pfk(Conc_Substrate1 = CONC_F6P, Conc_Substrate2 = CONC_ATP_T0, Conc_Activator = CONC_ADP_T0,Conc_Inhibitor = CONC_PEP, Ka = KA_ADP_PFK, Ki = KI_PEP_PFK, Conc_Enzyme = CONC_PFK, kcat = KCAT_PFK_ACTIVE, KM1 = KM_PFK_ACTIVE_F6P, KM2 = KM_PFK_ACTIVE_ATP):
...
def pyruvateKinase(Conc_Substrate1 = CONC_PEP, Conc_Substrate2 = CONC_ADP_T0, Conc_Inhibitor = CONC_ATP_T0,Conc_Activator = CONC_F16P, Ka= KA_F16BP_PK, Ki = KI_ATP_PK, Conc_Enzyme = CONC_PYRUVATE_KINASE, kcat = KCAT_PK, KM1 = KM_PK_PEP, KM2 = KM_PK_ADP):
...
def run():
    ...
    for step in range(1,TOTAL_STEPS):
        ...
        # ATP
        arr2DCounts[step][4] = CONC_ATP_T0#max(0, arr2DCounts[step-1][4] + 2*arr2DEnzymeStepTurnover[step][3] - arr2DEnzymeStepTurnover[step][1] - arr2DEnzymeStepTurnover[step][0])
        # ADP
        arr2DCounts[step][5] = CONC_ADP_T0#max(0, arr2DCounts[step-1][5] - 2*arr2DEnzymeStepTurnover[step][3] + arr2DEnzymeStepTurnover[step][1] + arr2DEnzymeStepTurnover[step][0])
