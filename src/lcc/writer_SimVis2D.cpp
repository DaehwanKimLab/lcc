...
//    ofs << in+ in+ in+ in+ "for y in range(0, Data.shape[1], self.ReductionFactor):" << endl;
    ...
    ofs << in+ in+ in+ "MatrixFoldReduction = SimM.State.FoldReduction" << endl;
    ofs << in+ in+ in+ "SamplingInterval = int(MatrixFoldReduction / self.ReductionFactor)" << endl;
    ofs << in+ in+ in+ "for x in range(0, Data.shape[0] - 1):" << endl;
    ofs << in+ in+ in+ in+ "for y in range(0, Data.shape[1] - 1):" << endl;
    ofs << in+ in+ in+ in+ in+ "for i in range(SamplingInterval):" << endl;
    ofs << in+ in+ in+ in+ in+ in+ "Step = i * (1 / SamplingInterval)" << endl;
    ofs << in+ in+ in+ in+ in+ in+ "RawMolLevel = SimF.BilinearInterpolation(Data, x + Step, y + Step)" << endl;
    ofs << in+ in+ in+ in+ in+ in+ "PercentMolLevel = SimF.BilinearInterpolation(Data_Normalized, x + Step, y + Step)" << endl;
    ofs << in+ in+ in+ in+ in+ in+ "bSkip = self.EvaluateToSkip(RawMolLevel, PercentMolLevel)" << endl;
    ofs << in+ in+ in+ in+ in+ in+ "if bSkip:" << endl;
    ofs << in+ in+ in+ in+ in+ in+ in+ "continue" << endl;
    ofs << endl;
    ofs << in+ in+ in+ in+ in+ in+ "intensity = int(np.floor(PercentMolLevel * self.MaxBrightness))" << endl;
    ...
    ofs << in+ in+ in+ in+ in+ in+ "pygame.draw.rect(Screen, color, ((int(x + Step) * 10, int(y + Step) * 10), (self.ReductionFactor, self.ReductionFactor)))" << endl;
    ofs << endl;
    ofs << in+ in+ in+ in+ in+ in+ "if self.bContourLine:" << endl;
    ofs << in+ in+ in+ in+ in+ in+ in+ "if not self.bMaxStatic:" << endl;
    ofs << in+ in+ in+ in+ in+ in+ in+ in+ "self.SetContourLinePoints(Max=np.Max(Data))" << endl;
    ofs << in+ in+ in+ in+ in+ in+ in+ "if self.CheckContourLine(Data[x][y]):" << endl;
    ofs << in+ in+ in+ in+ in+ in+ in+ in+ "pygame.draw.rect(Screen, BLACK, ((x, y), (self.ReductionFactor, SamplingInterval)))" << endl;
    ofs << endl;
