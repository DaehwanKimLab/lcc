...
def Write_CellProcess(Writer, Comp, ProGen, ProcessID):
    ...
    # PAP I accounts for ~90% of the poly(A) tails in E. coli, which are on average 15–30 nt long [10,12,20,48]. However, a limited number of the poly(A) tails can be longer than 30 nt, while the bulk of the tails are less than 10 nt in length.
    Len_PolyA = 20
    ...
    with Writer.Statement("class F%s(FCellProcess):" % ProcessID):
        ...
        with Writer.Statement("def CleaveRNAs(self, Idx_Master_RNA, Rate_RNACleavage):"):
            ...
            Writer.Statement("Idx_RndRNAsToCleave = self.PickRandomIndexFromPool_Weighted(N_RNAsToCleave, Idx_Master_RNA, Count_RNAs)")
